...
import { 
  ...
  Flame, MessageSquare, Newspaper, Film, CreditCard, Folder 
} from 'lucide-react';
...
export default function BoardCard({ 
  ...
  const getBoardIcon = (name) => {
    ...
    if (n.includes('DEV') || n.includes('CODE') || n.includes('PROGRAM') || n.includes('TECH')) return <Code className="w-4 h-4 text-indigo-400" />;
    if (n.includes('AI') || n.includes('TOOL') || n.includes('GPT') || n.includes('MODEL')) return <Cpu className="w-4 h-4 text-purple-400" />;
    if (n.includes('ROUTINE') || n.includes('DAILY') || n.includes('MAIN') || n.includes('AUTOMATION')) return <Flame className="w-4 h-4 text-amber-400" />;
    ...
    if (n.includes('MOVIE') || n.includes('FILM') || n.includes('SHOW') || n.includes('DESIGN')) return <Film className="w-4 h-4 text-rose-400" />;
  ...
  return (
    <div className="board-card p-6 flex flex-col group/board">
      {/* Top Accent Line */}
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2.5 min-w-0">
          ...
          <h3 className="text-sm font-black tracking-wide text-white uppercase truncate">
            {boardName}
          </h3>
          <span className="px-2 py-0.5 text-[11px] font-extrabold rounded-full bg-emerald-500/10 text-emerald-400 border border-emerald-500/20">
            {bookmarks.length}
          </span>
        </div>
      ...
      <div className="h-px bg-gradient-to-r from-white/20 via-white/10 to-transparent my-3.5" />
      ...
      <div className="flex flex-col gap-1.5 min-h-[30px]">
        ...
        {bookmarks.length === 0 && (
          <div 
            onClick={() => onAddLinkToBoard(boardName)}
            className="py-4 px-3 rounded-xl border border-dashed border-white/10 hover:border-emerald-500/40 text-center cursor-pointer text-xs text-gray-500 hover:text-emerald-400 transition-all mt-1 flex items-center justify-center gap-2"
          >
            <Plus className="w-4 h-4" />
